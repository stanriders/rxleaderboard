...
export const Score: FC<Props> = (props) => {
  ...
  if (props.simple){
    return <Card className="min-w-72 bg-default-100 dark:bg-default-50" shadow="sm" fullWidth key={score.id}>
    {props.showPlayer ? (
      <>
    ...
    <CardBody className={`p-2 ${opacity} bg-content1 rounded-large`}>
      <div className="flex gap-2 items-center justify-center">
        <div className="block flex-none text-center w-10"><Link isExternal className="text-default-500 text-md" href={`https://osu.ppy.sh/scores/${score.id}`}>{score.grade}</Link></div>
        <div className="flex flex-col flex-auto min-w-1">
          <div><Link href={`/beatmaps/${score.beatmapId}`} className="text-primary-500 text-xs" size="sm">{beatmap.artist} - {beatmap.title}<Spacer x={2}/><span className="text-primary-300 text-xs pr-3">{beatmap.difficultyName}</span></Link></div>
          <Tooltip showArrow content={new Date(score.date).toLocaleString()}><div className="hidden md:block w-fit truncate text-default-400 text-xs">{formatDistance(new Date(score.date), new Date(), { addSuffix: true })}</div></Tooltip>
        </div>
        ...
        <div className="hidden md:block flex-none text-primary-400 items-center text-center w-24 text-md font-semibold">{pp}</div>
      </div>
    </CardBody>
    <CardFooter className={`md:hidden flex gap-2 items-center justify-center ${opacity}`}>
    <div className="flex-none text-center w-10"><Tooltip showArrow content={new Date(score.date).toLocaleString()}><div className="w-fit truncate text-default-400 text-xs">{formatDistance(new Date(score.date), new Date(), { addSuffix: true })}</div></Tooltip></div>
      <div className="flex-auto text-default-500 text-right text-xs">{score.mods?.map(m => <Mod key={m} mod={m}/>)}</div>
      <div className="flex-none text-default-500 text-center text-xs">{(score.accuracy * 100)?.toFixed(2) ?? (<>- </>)}%</div>
      <div className="flex-none text-primary-300 items-center text-center w-20 text-sm font-semibold">{pp}</div>
    </CardFooter>
  </Card>
  }
