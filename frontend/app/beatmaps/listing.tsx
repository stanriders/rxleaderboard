...
const fetcher = (url : any) => fetch(url).then(r => r.json()).catch(error => console.log("Beatmap listing fetch error: " + error));;
...
export default function BeatmapListing() {
    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {response.beatmaps?.map((row: ListingBeatmap, index: number) => (
            <BeatmapCard beatmap={row} key={row.id}/>
        ))}
      </div>
      <Spacer y={4} />
