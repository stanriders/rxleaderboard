...
namespace LazerRelaxLeaderboard.Services
{
    public class DiscordBackgroundService : BackgroundService
    {
        ...
        public DiscordBackgroundService(IOptions<DiscordConfig> configuration, ILogger<DiscordBackgroundService> logger, DiscordSocketClient client)
        {
            _logger = logger;
            _client = client;
    ...
    public class DiscordService : IDiscordService
    {
        ...
        public async Task PostBestPlayerAnnouncement(int userId)
        {
            ...
            await verifiedChannel.SendMessageAsync(
                embed: new EmbedBuilder()
                    .WithAuthor($"{user.Username} - {user.TotalPp:N2}pp", $"https://a.ppy.sh/{user.Id}", $"https://rx.stanr.info/users/{user.Id}")
